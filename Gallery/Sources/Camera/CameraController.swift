...
class CameraController: UIViewController {
  ...
  @objc func rotateButtonTouched(_ button: UIButton) {
    UIView.animate(withDuration: 0.3, animations: {
      ...
      self.cameraMan.switchCamera {
        self.hideRotateOverlayView()
      }
    })
  }

    private func hideRotateOverlayView() {
  ...
  @objc func shutterButtonTouched(_ button: ShutterButton) {
    ...
    cameraMan.takePhoto(location: locationManager?.latestLocation) { [weak self] asset in
  ...
  func refreshView() {
    ...
    cameraView.bottomView.gFade(visible: hasImages)
  }
