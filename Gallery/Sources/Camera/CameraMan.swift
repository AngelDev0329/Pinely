...
class CameraMan {
  ...
  func takePhoto(location: CLLocation?, completion: @escaping ((PHAsset?) -> Void)) {
    ...
    queue.async {
      self.stillImageOutput?.captureStillImageAsynchronously(from: connection) {
        buffer, error in
          self.processImageCapture(buffer: buffer, error: error,
                                   location: location, completion: completion)
      }
    }
  }

    private func processImageCapture(buffer: CMSampleBuffer?, error: Error?,
                                     location: CLLocation?, completion: @escaping ((PHAsset?) -> Void)) {
        if error == nil, let buffer = buffer, CMSampleBufferIsValid(buffer),
