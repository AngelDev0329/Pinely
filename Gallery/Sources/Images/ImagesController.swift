...
class ImagesController: UIViewController {
  ...
  func setup() {
    ...
    dropdownController.view.gPin(on: .left)
    dropdownController.view.gPin(on: .right)
    dropdownController.view.gPin(on: .height, constant: -40) // subtract gridView.topView height

    dropdownController.expandedTopConstraint = dropdownController.view.gPin(on: .top, view: gridView.topView, on: .bottom, constant: 1)
    dropdownController.expandedTopConstraint?.isActive = false
    dropdownController.collapsedTopConstraint = dropdownController.view.gPin(on: .top, on: .bottom)
  ...
  func show(album: Album) {
    ...
    gridView.collectionView.gScrollToTop()
  ...
  func refreshView() {
    ...
    gridView.bottomView.gFade(visible: hasImages)
    gridView.collectionView.gUpdateBottomInset(hasImages ? gridView.bottomView.frame.size.height : 0)
  }
...
extension ImagesController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
  ...
  func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
    ...
    if Config.Camera.singleImageMode {
        while !cart.images.isEmpty {
