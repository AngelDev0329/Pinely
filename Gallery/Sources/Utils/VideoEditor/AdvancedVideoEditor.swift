...
public class AdvancedVideoEditor: VideoEditing {
  ...
  fileprivate func wireVideo(_ avAsset: AVAsset) {
    ...
    if !videoTracks.isEmpty {
      // Output
      let wVideoOutput = AVAssetReaderVideoCompositionOutput(videoTracks: videoTracks, videoSettings: nil)
      wVideoOutput.videoComposition = EditInfo.composition(avAsset)
      if reader.canAdd(wVideoOutput) {
        reader.add(wVideoOutput)
      }

      // Input
      let wVideoInput = AVAssetWriterInput(mediaType: AVMediaType.video,
                                          outputSettings: EditInfo.videoSettings,
                                          sourceFormatHint: avAsset.g_videoDescription)
      if writer.canAdd(wVideoInput) {
        writer.add(wVideoInput)
      }

      self.videoInput = wVideoInput
      self.videoOutput = wVideoOutput
    }
  }
  ...
  fileprivate func wireAudio(_ avAsset: AVAsset) {
    ...
    if !audioTracks.isEmpty {
      // Output
      let wAudioOutput = AVAssetReaderAudioMixOutput(audioTracks: audioTracks, audioSettings: nil)
      wAudioOutput.alwaysCopiesSampleData = true
      if reader.canAdd(wAudioOutput) {
        reader.add(wAudioOutput)
      }

      // Input
      let wAudioInput = AVAssetWriterInput(mediaType: AVMediaType.audio,
                                          outputSettings: EditInfo.audioSettings,
                                          sourceFormatHint: avAsset.g_audioDescription)
      if writer.canAdd(wAudioInput) {
        writer.add(wAudioInput)
      }

      self.audioOutput = wAudioOutput
      self.audioInput = wAudioInput
    }
  }
