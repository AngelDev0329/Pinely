...
class VideosController: UIViewController {
  ...
  func setup() {
    ...
    videoBox.gPin(size: CGSize(width: 44, height: 44))
    videoBox.gPin(on: .centerY)
    videoBox.gPin(on: .left, constant: 38)

    infoLabel.gPin(on: .centerY)
    infoLabel.gPin(on: .left, view: videoBox, on: .right, constant: 11)
    infoLabel.gPin(on: .right, constant: -50)
    ...
    gridView.arrowButton.updateText("Gallery.AllVideos".gLocalize(fallback: "ALL VIDEOS"))
  ...
  func refreshView() {
    if let selectedItem = cart.video {
      videoBox.imageView.gLoadImage(selectedItem.asset)
    } else {
      videoBox.imageView.image = nil
    }

    let hasVideo = (cart.video != nil)
    gridView.bottomView.gFade(visible: hasVideo)
    gridView.collectionView.gUpdateBottomInset(hasVideo ? gridView.bottomView.frame.size.height : 0)
  ...
  func makeVideoBox() -> VideoBox {
    let newVideoBox = VideoBox()
    newVideoBox.delegate = self
  ...
  func makeInfoLabel() -> UILabel {
    ...
    label.text = String(format: "Gallery.Videos.MaxiumDuration".gLocalize(fallback: "FIRST %d SECONDS"),
