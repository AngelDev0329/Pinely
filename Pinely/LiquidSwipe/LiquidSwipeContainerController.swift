...
public protocol LiquidSwipeContainerDelegate: AnyObject {
    ...
    func liquidSwipeContainer(_ liquidSwipeContainer: LiquidSwipeContainerController,
                              didFinishTransitionTo: UIViewController, transitionCompleted: Bool)
}
...
open class LiquidSwipeContainerController: UIViewController {
    ...
    @IBOutlet weak var vControlsView: UIView!
    ...
    private var initialHorRadius: CGFloat = 40.0 // 48.0
    private var maxHorRadius: CGFloat {
        return view.bounds.width * 0.8
    }

    private var initialVertRadius: CGFloat = 110.0 // 82.0
    ...
    private var initialWaveCenter: CGFloat {
        return view.bounds.height * 0.61 // 0.7167487685
    }
    private var animationStartTime: CFTimeInterval?
    private var animating: Bool = false
    private var duration: CFTimeInterval = 0.8

    @IBOutlet var leftEdgeGesture: UIPanGestureRecognizer!
                      ...
                      let time = animation?.elapsedTime else {
                        if let viewController = self.nextViewController {
                            self.delegate?.liquidSwipeContainer(
                                self,
                                didFinishTransitionTo: viewController,
    ...
    @IBAction func leftEdgePan(_ sender: UIPanGestureRecognizer) {
        ...
        if sender.state == .began {
                ...
                if !(self.shouldFinish || self.shouldCancel) {
                    ...
                    default:
                        if progress <= 0.6 {
                            ...
                            self.shouldCancel = true
                            self.animationStartTime = CACurrentMediaTime() -
                                CFTimeInterval(CGFloat(self.duration) * progress)
                        }
                    }
                }
                let cTime = (animation?.currentTime ?? CACurrentMediaTime()) -
                    (self.animationStartTime ?? CACurrentMediaTime())
                if self.shouldFinish {
                    let progress = self.animationProgress - CGFloat(cTime/self.duration)
                    let direction: CGFloat = (self.initialWaveCenter - mask.waveCenterY).sign == .plus ? 1 : -1
            ...
            previousViewAnimation?.completionBlock = { (_, _) in
                self.animating = false
                if self.shouldFinish {
                ...
                if self.shouldCancel,
                    ...
                    self.delegate?.liquidSwipeContainer(self,
                                                        didFinishTransitionTo: viewController,
                                                        transitionCompleted: false)
                }
            }
    ...
    private func layoutPageView(_ page: UIView) {
        ...
        page.topAnchor.constraint(equalTo: vContentView.topAnchor).isActive = true
        page.bottomAnchor.constraint(equalTo: vContentView.bottomAnchor).isActive = true
        page.leadingAnchor.constraint(equalTo: vContentView.leadingAnchor).isActive = true
        page.widthAnchor.constraint(equalTo: vContentView.widthAnchor).isActive = true
    }
    ...
    private func configureInitialState() {
        ...
        vContentView.addSubview(firstPage)
    ...
    private func configureNextPage() {
        ...
        if let currentPage = currentPage {
            vContentView.insertSubview(page, belowSubview: currentPage)
        } else {
            vContentView.addSubview(page)
        }
        layoutPageView(page)
    }
    ...
    private func configurePreviousPage() {
        ...
        if let currentPage = currentPage {
            vContentView.insertSubview(page, aboveSubview: currentPage)
        } else {
            vContentView.addSubview(page)
        }
        layoutPageView(page)
...
private extension LiquidSwipeContainerController {
    ...
    private func sideWidth(forProgress progress: CGFloat) -> CGFloat {
        ...
        let pp2 = view.bounds.width - initialSideWidth
        let p3 = (progress - p1)/(p2 - p1)
        return initialSideWidth + pp2 * p3
    }
}
