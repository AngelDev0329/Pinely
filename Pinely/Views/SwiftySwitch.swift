...
public class SwiftySwitch: UIView {
    ...
    internal weak var delegate: SwiftySwitchDelegate?
    ...
    @IBInspectable public var myColor: UIColor =
        UIColor(red: 35/255, green: 110/255, blue: 129/255, alpha: 1.0) {
        didSet {
            config()
        }
    }
    ...
    @IBInspectable public var dotOffColor: UIColor = UIColor(red: 0/255, green: 66/255, blue: 99/255, alpha: 1.0) {
        didSet {
            config()
        }
    }
    @IBInspectable public var dotOnColor: UIColor = UIColor(red: 0/255, green: 199/255, blue: 170/255, alpha: 1.0) {
        didSet {
            config()
        }
    }
    @IBInspectable public var smallDotColor: UIColor = UIColor(red: 227/255, green: 49/255, blue: 67/255, alpha: 1.0) {
        didSet {
            config()
        }
    }
...
private class SwitchBall: UIView {
    ...
    fileprivate init(_ onColor: UIColor, _ offColor: UIColor,
                     _ dotColor: UIColor, _ isOn: Bool, _ myFrame: CGRect,
                     _ dotSpace: Int, _ smallDotMultiplier: CGFloat,
                     _ dotTravelTime: TimeInterval) {
        ...
        if isOn {
            super.init(frame: CGRect(x: myFrame.width - ballDiameter - dotSpacer,
                                     y: dotSpacer, width: ballDiameter, height: ballDiameter))
            self.backgroundColor = onColor
        } else {
            super.init(frame: CGRect(x: dotSpacer, y: dotSpacer,
        ...
        NSLayoutConstraint(item: centerBall as Any, attribute: .width,
                           relatedBy: .equal, toItem: nil, attribute: .notAnAttribute,
                           multiplier: 1, constant: centerBallDiameter).isActive = true
        NSLayoutConstraint(item: centerBall as Any, attribute: .height,
                           relatedBy: .equal, toItem: nil, attribute: .notAnAttribute,
                           multiplier: 1, constant: centerBallDiameter).isActive = true
        NSLayoutConstraint(item: centerBall as Any, attribute: .centerX,
                           relatedBy: .equal, toItem: self, attribute: .centerX,
                           multiplier: 1, constant: 0).isActive = true
        NSLayoutConstraint(item: centerBall as Any, attribute: .centerY,
    ...
    func turnOn(completion: @escaping () -> Void) {
        ...
        UIView.animate(withDuration: dotTravelTime) { [weak self] in
            if self != nil {
                ...
                let d1 = (self!.myFrame.width - self!.ballDiameter - (self!.dotSpacer * 2))
                self?.frame.origin.x = self!.dotSpacer + d1
                self?.layoutIfNeeded()
            }
        ...
        UIView.animate(
            withDuration: (1 - Double(smallDotMultiplier!)) * dotTravelTime,
            delay: Double(smallDotMultiplier!) * dotTravelTime,
...
protocol SwiftySwitchDelegate: AnyObject {
